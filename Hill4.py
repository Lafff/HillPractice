...
def solve(open_text,key):
	key_matrica = np.array(([key[0],key[1],key[2],key[3]],[key[4],key[5],key[6],key[7]],[key[8],key[9],key[10],key[11]],[key[12],key[13],key[14],key[15]]))
	obr_matrix = np.linalg.inv(key_matrica)
	obr_matrix %= 64
	obr_matrix = np.round(obr_matrix).astype(int)

	result= '<table align="center" cellspacing=30px"><tr><th>Ключевая матрица</th><th>Обратная матрица</th></tr> <tr><td>'+str(key_matrica[0]) + '<p>' + str(key_matrica[1]) + '<p>'+ str(key_matrica[2])+ '<p>'+ str(key_matrica[3])+'</td><td>'+str(obr_matrix[0]) + '<p>' + str(obr_matrix[1]) + '<p>'+ str(obr_matrix[2])+ '<p>'+ str(obr_matrix[3])+'</td></tr></table>'
	closed_text = ''
	decoded_text = ''

	if len(open_text) % 4 != 0:
		open_text += ' ' * (4-(len(open_text) % 4))

	result += '<div style="text-align:center;">!!!-------------------КОДИРОВАНИЕ-------------------!!!</div><br>'
	for i in range(0, len(open_text),+4):
		cycle_count = 1
		block = np.array((dict[open_text[i]], dict[open_text[i+1]],dict[open_text[i+2]],dict[open_text[i+3]]))
		result+= f'<p><span style="color:#ed9393;">Текущий блок {cycle_count}: </span>' + str(block[0]) + ' '+ str(block[1]) + ' '+ str(block[2]) + ' '+ str(block[3]) + '</p>'
	...
	for i in range(0, len(closed_text),+4):
		cycle_count = 1
